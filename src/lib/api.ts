...
// Reads VITE_API_BASE_URL; when unset or unreachable, callers get a typed
// error and render honest empty states — never fake data.
...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  if (!BASE) {
    throw {
      status: 0,
      message: "Not connected to Growzzy yet — connect your backend to see live data.",
    } satisfies ApiError;
  }
  let res: Response;
  try {
...
// -------- Endpoint helpers (typed stubs, ready for real backend) --------
